...
A simple flask backend that has a search and graphing functionality (functionality imported from previous CLI)
Very basic react front end that consists of textfields which are to be filled with their respective value or 
left empty for a "no filter" option. If the graphing form is selected, the user should enter a subject to graph 
e.g. 'CIS' or 'HIST'.
...
post subject graph : http://131.104.49.112/api/graph
