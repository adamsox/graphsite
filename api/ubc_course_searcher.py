...
def check_year(course):
    course_year = re.split('(\d+)', course['cc'])

    if year == course_year[1][0]:
        return True
    else:
        return False
...
def open_courses():
    ...
    with open("ubc_courses.json", "r") as file:
